...
public class Main {
	...
	private static void sanityCheck() {
		...
		ArrayList<Object> valuesx = new ArrayList<Object>();
		valuesx.add("ABRA");
		valuesx.add(3);
		valuesx.add(2 == 2);
		db.insert("Students1", valuesx);
		ArrayList<Object> values = new ArrayList<Object>();
		values.add("Ion"+20);
		values.add(20);
		values.add(20%2==1);
		db.update("Students1", values, "");
		HashMap<String, SingleTable> hmap = ((Database) (db)).getHmap();
		for (String name: hmap.keySet()){

            String key =name.toString();
            SingleTable value = hmap.get(name);
            
            System.out.println(key+ " ");
            for(int i = 0; i< value.getValues().size(); i++)
            {
            	ArrayList<Object> val = value.getValue(i);
